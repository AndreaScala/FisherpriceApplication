 ...
 * @author ilario
 */
public class GossipCheckThread2 implements Runnable{
    private int[] counters = new int[SubsetOfNodes.size()];
    public String name;
    public long period;

    public GossipCheckThread2(String name) {
        this.name = name;
        this.period = Period;
    }
    ...
    public void run() {
        while (true) {
            try {
                ...
                Logger.getLogger(GossipCheckThread2.class.getName()).log(Level.SEVERE, null, ex);
            }
            if(working) {
